...
#now create dataframe for merging from 2004 to 2020
  ...
  # jsonlite::fromJSON(paste('https://unstats.un.org/SDGAPI/v1/sdg/Series/Data?seriesCode=',series,'&timePeriodStart=',start,'&timePeriodEnd=',end,'&pageSize=10000',sep=""), flatten = TRUE)$data %>%
    ...
    mutate(available=!is.na(value),  #check whether indicator is available
           quality=case_when(
             ...
             (available & is.na(attributes.Nature)) ~ 1,
             (available & (attributes.Nature=='E' | attributes.Nature=='G')) ~ 1,
             (available & attributes.Nature=='M') ~ 0,
             attributes.Nature=='N' ~ as.numeric(NA), #these indicators aren't relevenat for countries.
             !available ~ 0
           )) %>%
    group_by(date) %>%
...
oecd_df <- read_csv(paste(raw_dir,'/oecd_data/country_data/OECD_data_scored.csv', sep=""))
...
#overall sumstats
  stats_sum <- un_sdg_df %>%
    ...
    select(goal,  ind_available, ind_quality)
          ...
          DT::datatable(stats_sum, caption="Table of Overall Scores by Goal in 2020",
                      rownames=FALSE,
        ...
        DT::datatable(stats_df, caption="Table of Scores by Country in 2020",
                      rownames=FALSE,
